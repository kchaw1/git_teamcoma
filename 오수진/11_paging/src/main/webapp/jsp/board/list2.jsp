			...
			<nav>
			  <ul class="pagination">
			    <li <c:if test="${prev eq false}">class="disabled"</c:if> >
			      <a    <c:choose>	 
			      			<c:when test="${prev eq true}">href="list2.do?pageNo=${beginPage - 1}"</c:when> 
			    ...
			    <li <c:if test="${next eq false}">class="disabled"</c:if> >
			      <a    <c:choose>	 
			      			<c:when test="${next eq true}">href="list2.do?pageNo=${endPage + 1}"</c:when> 
			      			<c:otherwise>href="#${endPage + 1}"</c:otherwise>
			      		</c:choose>
			      		aria-label="Next">
